...
def box(ax, x, y, w, h, color, label, sublabel=""):
    ...
    if sublabel:
        ax.text(x + w / 2, y + h / 2 - 0.2, sublabel,
                ha="center", va="center", fontsize=7, color=COLORS["subtitle"])
...
# Title
ax.text(8, 8.5, "Energy-Seer — System Architecture", ha="center", va="center",
...
# FastAPI layer
box(ax, 3.0, 7.0, 3.5, 0.9, COLORS["api"], "FastAPI /api/v1/", "predict · anomaly · drift · forecast")
box(ax, 3.0, 5.8, 1.6, 0.9, COLORS["api"], "Rate Limiter", "300 req/min")
box(ax, 4.7, 5.8, 1.8, 0.9, COLORS["api"], "Correlation ID", "Middleware")

# ML Model
box(ax, 7.5, 6.5, 3.5, 1.4, COLORS["model"], "Ensemble Model",
    "XGBoost + LightGBM + RF\n5-fold CV · R² / RMSE")

# Feature Pipeline
box(ax, 7.5, 4.8, 3.5, 1.4, COLORS["model"], "Feature Pipeline",
    "Lag · Rolling · Temporal\nWeather Ratios · Building Enc.")

# Monitoring
box(ax, 3.0, 4.0, 3.5, 1.5, COLORS["monitoring"], "Monitoring",
    "KS-test Drift · IsolationForest\nZ-score Anomaly · Pred Logging")

# RAG / FAISS
box(ax, 7.5, 3.2, 3.5, 1.3, COLORS["rag"], "RAG / FAISS",
...
# Airflow
box(ax, 3.0, 2.2, 3.5, 1.3, COLORS["pipeline"], "Airflow DAG",
    "Weekly Retrain · R² Gate\nRefresh Anomaly · Rebuild RAG")

# Docker
box(ax, 7.5, 1.5, 7.8, 0.9, COLORS["api"], "Docker + docker-compose",
...
legend_items = [
    ...
    mpatches.Patch(facecolor=COLORS["model"] + "55", edgecolor=COLORS["model"], label="ML / Features"),
    mpatches.Patch(facecolor=COLORS["monitoring"] + "55", edgecolor=COLORS["monitoring"], label="Monitoring"),
    mpatches.Patch(facecolor=COLORS["storage"] + "55", edgecolor=COLORS["storage"], label="Storage"),
    mpatches.Patch(facecolor=COLORS["rag"] + "55", edgecolor=COLORS["rag"], label="RAG / FAISS"),
    mpatches.Patch(facecolor=COLORS["pipeline"] + "55", edgecolor=COLORS["pipeline"], label="Airflow"),
]
