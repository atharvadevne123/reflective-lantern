...
def geometric_mean(values: list[float]) -> float:
    """Compute the geometric mean of a positive sequence.

    Args:
        values: Non-empty list of strictly positive numbers.

    Returns:
        Geometric mean value, rounded to 6 decimal places.
...
def harmonic_mean(values: list[float]) -> float:
    """Compute the harmonic mean of a positive sequence.

    Suitable for averaging rates and ratios.

    Args:
        values: Non-empty list of strictly positive numbers.

    Returns:
        Harmonic mean value, rounded to 6 decimal places.
...
def trimmed_mean(values: list[float], trim_pct: float = 0.1) -> float:
    """Compute the trimmed mean by removing the extreme *trim_pct* from each tail.

    Args:
        values: Numeric sequence to summarise.
        trim_pct: Fraction of values to remove from each tail (default 10%).

    Returns:
        Trimmed mean; falls back to arithmetic mean when too few values remain.

    Raises:
        ValueError: If *values* is empty or *trim_pct* is outside [0, 0.5).
    """
    if not values:
        raise ValueError("values must not be empty")
