...
def test_add_noise_length() -> None:
...
def test_add_noise_zero_seed_deterministic() -> None:
    from app.data.augmentor import add_noise
...
def test_bootstrap_sample_length(sample_records) -> None:
...
def test_bootstrap_sample_empty() -> None:
    from app.data.augmentor import bootstrap_sample

    assert bootstrap_sample([], n=5) == []


def test_augment_dataset_size(sample_records) -> None:
...
def test_augment_dataset_multiplier_one(sample_records) -> None:
    from app.data.augmentor import augment_dataset
