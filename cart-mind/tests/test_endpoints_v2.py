...
class TestBatchPredict:
    def test_batch_returns_200(self, client, intent_payload):
        payload = {"items": [intent_payload, intent_payload]}
        r = client.post("/api/v1/predict/batch", json=payload)
        assert r.status_code == 200

    def test_batch_count_matches_input(self, client, intent_payload):
        payload = {"items": [intent_payload] * 4}
        data = client.post("/api/v1/predict/batch", json=payload).json()
        assert data["count"] == 4
        assert len(data["predictions"]) == 4

    def test_batch_probabilities_in_range(self, client, intent_payload):
        payload = {"items": [intent_payload] * 3}
        data = client.post("/api/v1/predict/batch", json=payload).json()
        for pred in data["predictions"]:
            assert 0.0 <= pred["purchase_probability"] <= 1.0

    def test_batch_mean_probability_in_range(self, client, intent_payload):
        payload = {"items": [intent_payload] * 3}
        data = client.post("/api/v1/predict/batch", json=payload).json()
        assert 0.0 <= data["mean_probability"] <= 1.0

    def test_batch_echoes_ids(self, client, intent_payload):
    ...
    def test_oversized_batch_rejected(self, client, intent_payload):
        r = client.post("/api/v1/predict/batch", json={"items": [intent_payload] * 501})
        assert r.status_code == 422

    def test_batch_matches_single_prediction(self, client, intent_payload):
    ...
    def test_various_batch_sizes(self, client, intent_payload, n):
...
class TestModelInfo:
    ...
    def test_auc_is_null_or_valid(self, client):
        data = client.get("/api/v1/model/info").json()
...
class TestCacheStats:
    ...
    def test_cached_similar_response_is_identical(self, client, similar_payload):
