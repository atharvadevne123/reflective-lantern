...
class TestModelTraining:
    def test_train_returns_pipeline_and_metrics(self, feature_df, binary_labels):
    ...
    def test_auc_in_valid_range(self, feature_df, binary_labels):
        from app.model import train_model

        _, metrics = train_model(feature_df, binary_labels)
        assert 0.0 <= metrics["auc_mean"] <= 1.0

    def test_metrics_contains_required_keys(self, feature_df, binary_labels):
        from app.model import train_model
    ...
    def test_n_features_matches_input(self, feature_df, binary_labels):
        from app.model import train_model

        _, metrics = train_model(feature_df, binary_labels)
        assert metrics["n_features"] == len(_FEATURE_COLS)

    def test_model_file_created(self, trained_pipeline, tmp_path, monkeypatch):
...
class TestModelInference:
    def test_predict_returns_proba_and_labels(self, trained_pipeline, feature_df):
    ...
    def test_probas_in_range(self, trained_pipeline, feature_df):
        from app.model import predict_intent
    ...
    def test_labels_are_binary(self, trained_pipeline, feature_df):
        from app.model import predict_intent
    ...
    def test_predict_various_batch_sizes(self, trained_pipeline, n_rows):
        from app.features import make_sample_dataframe
        from app.model import predict_intent
...
class TestFaissIndex:
    ...
    def test_similarity_scores_positive(self):
        from app.model import build_faiss_index, search_similar_items
