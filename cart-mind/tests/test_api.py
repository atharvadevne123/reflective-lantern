...
class TestPredictEndpoint:
    def test_predict_returns_200(self, client, intent_payload):
        r = client.post("/api/v1/predict", json=intent_payload)
        assert r.status_code == 200

    def test_predict_probability_range(self, client, intent_payload):
        data = client.post("/api/v1/predict", json=intent_payload).json()
        assert 0.0 <= data["purchase_probability"] <= 1.0

    def test_predict_returns_boolean_label(self, client, intent_payload):
        data = client.post("/api/v1/predict", json=intent_payload).json()
        assert isinstance(data["will_purchase"], bool)

    def test_predict_confidence_values(self, client, intent_payload):
        data = client.post("/api/v1/predict", json=intent_payload).json()
        assert data["confidence"] in ("high", "medium")

    def test_predict_correlation_id_returned(self, client, intent_payload):
        data = client.post("/api/v1/predict", json=intent_payload).json()
        assert "correlation_id" in data
        assert len(data["correlation_id"]) > 0

    @pytest.mark.parametrize("age", [0, 18, 65, 120])
    def test_predict_edge_ages(self, client, intent_payload, age):
        payload = {**intent_payload, "user_age": age}
        r = client.post("/api/v1/predict", json=payload)
        assert r.status_code == 200

    def test_predict_invalid_age_rejected(self, client, intent_payload):
        payload = {**intent_payload, "user_age": 150}
        r = client.post("/api/v1/predict", json=payload)
        assert r.status_code == 422

    def test_predict_missing_user_id_rejected(self, client, intent_payload):
...
class TestRecommendEndpoint:
    ...
    def test_recommend_count_matches_top_k(self, client, recommend_payload):
        data = client.post("/api/v1/recommend", json=recommend_payload).json()
        assert data["count"] == len(data["recommendations"])

    def test_recommend_scores_in_range(self, client, recommend_payload):
        data = client.post("/api/v1/recommend", json=recommend_payload).json()
        for rec in data["recommendations"]:
            assert 0.0 <= rec["score"] <= 1.0

    def test_recommend_top_k_respected(self, client, recommend_payload):
        payload = {**recommend_payload, "top_k": 3}
        data = client.post("/api/v1/recommend", json=payload).json()
        assert data["count"] == 3

    def test_recommend_has_rank_field(self, client, recommend_payload):
        data = client.post("/api/v1/recommend", json=recommend_payload).json()
...
class TestSimilarEndpoint:
    def test_similar_returns_200(self, client, similar_payload):
        r = client.post("/api/v1/similar", json=similar_payload)
        assert r.status_code == 200

    def test_similar_count_matches_top_k(self, client, similar_payload):
        data = client.post("/api/v1/similar", json=similar_payload).json()
        assert data["count"] == len(data["similar_items"])

    def test_similar_similarity_scores_positive(self, client, similar_payload):
        data = client.post("/api/v1/similar", json=similar_payload).json()
        for item in data["similar_items"]:
            assert item["similarity_score"] >= 0.0

    def test_similar_seed_id_echoed(self, client, similar_payload):
        data = client.post("/api/v1/similar", json=similar_payload).json()
...
class TestDriftEndpoint:
    ...
    def test_drift_result_keys(self, client):
        import numpy as np
    ...
    def test_drift_empty_feature_dict(self, client):
