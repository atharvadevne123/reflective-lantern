...
class TestTTLExpiry:
    ...
    def test_entry_survives_within_ttl(self) -> None:
...
class TestLRUEviction:
    ...
    def test_recently_set_key_survives_eviction(self) -> None:
...
class TestCacheStats:
    ...
    def test_empty_cache_hit_rate_is_zero(self) -> None:
        assert TTLCache().stats()["hit_rate"] == 0.0

    @pytest.mark.parametrize("n", [1, 10, 100])
    def test_entry_count_tracks_inserts(self, n) -> None:
...
class TestTTLCacheDeleteAndSize:
    ...
    def test_delete_absent_key_returns_false(self) -> None:
        cache = TTLCache()
    ...
    def test_size_decrements_after_delete(self) -> None:
        cache = TTLCache()
        cache.set("x", 10)
        cache.delete("x")
        assert cache.size() == 0
    ...
    def test_size_after_partial_delete(self, n_delete) -> None:
