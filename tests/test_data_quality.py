...
def test_missing_hour_deducts_points():
    rec = {**GOOD_RECORD}
    del rec["hour"]
...
def test_invalid_hour_deducts_points():
...
def test_negative_kwh_deducts_points():
...
def test_extreme_kwh_deducts_minor_points():
...
def test_invalid_humidity_deducts_points():
...
def test_batch_score_returns_same_length():
    records = [GOOD_RECORD, {**GOOD_RECORD, "hour": 99}]
    result = batch_score(records)
    assert len(result) == 2


def test_quality_summary_mean_score():
...
def test_flag_outliers_detects_extreme_value():
    records = [
...
def test_flag_outliers_empty_when_constant():
    records = [{"kwh": 10.0}] * 5
    outliers = flag_outliers(records, "kwh")
    assert outliers == []


def test_flag_outliers_missing_field_skipped():
...
def test_valid_hours_score_100(hour):
