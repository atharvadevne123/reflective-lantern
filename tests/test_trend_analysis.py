...
def test_linear_trend_perfect_r_squared():
...
def test_percentage_change_zero_old():
...
def test_rolling_mean_window_2():
...
def test_detect_change_points_sharp_jump():
    values = [10.0] * 5 + [100.0] * 5
    cps = detect_change_points(values, threshold=1.5)
    assert 5 in cps


def test_detect_change_points_constant_series():
    values = [5.0] * 10
...
def test_seasonal_decompose_returns_keys():
...
def test_seasonal_decompose_length_matches():
...
def test_seasonal_decompose_short_input():
    values = [1.0, 2.0, 3.0]
    result = seasonal_decompose_naive(values, period=6)
...
def test_linear_trend_direction_parametrize(n, expected_dir):
    if n > 0:
