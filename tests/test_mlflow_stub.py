...
def test_log_training_run_mlflow_not_installed_returns_none():
    with patch.dict("os.environ", {"MLFLOW_TRACKING_URI": "http://localhost:5000"}), patch("builtins.__import__", side_effect=ImportError("mlflow not installed")):
        result = log_training_run({"lr": 0.01}, {"r2": 0.85})
    assert result is None


def test_log_training_run_mlflow_exception_returns_none():
    with patch.dict("os.environ", {"MLFLOW_TRACKING_URI": "http://localhost:5000"}), patch("app.mlflow_stub.os.getenv", return_value="http://localhost:5000"):
...
def test_upload_existing_file(tmp_path):
    ...
    mock_client = MagicMock()
    with patch("app.aws_stub._s3_client", return_value=mock_client), patch.object(s, "_BUCKET", "test-bucket"):
