...
def test_tracking_disabled_without_uri(monkeypatch) -> None:
    monkeypatch.setattr(tracking, "MLFLOW_TRACKING_URI", "")
    assert tracking.is_tracking_enabled() is False


def test_track_run_is_noop_when_disabled(monkeypatch) -> None:
...
def test_log_metrics_noop_when_disabled(monkeypatch) -> None:
    monkeypatch.setattr(tracking, "MLFLOW_TRACKING_URI", "")
    tracking.log_metrics({"accuracy": 0.9})  # must not raise


def test_log_params_noop_when_disabled(monkeypatch) -> None:
    monkeypatch.setattr(tracking, "MLFLOW_TRACKING_URI", "")
...
def test_s3_disabled_without_bucket(monkeypatch) -> None:
    monkeypatch.setattr(storage, "S3_BUCKET", "")
    assert storage.is_s3_enabled() is False


def test_upload_returns_none_when_disabled(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(storage, "S3_BUCKET", "")
...
def test_download_returns_false_when_disabled(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(storage, "S3_BUCKET", "")
    assert storage.download_artifact(str(tmp_path / "model.joblib")) is False


def test_s3_key_uses_prefix_and_basename(monkeypatch) -> None:
...
def test_upload_missing_file_returns_none(monkeypatch) -> None:
...
def test_upload_swallows_aws_error(monkeypatch, tmp_path) -> None:
...
def test_log_metrics_tolerates_non_numeric(monkeypatch, metrics) -> None:
    monkeypatch.setattr(tracking, "MLFLOW_TRACKING_URI", "")
