...
class TestGenerateSampleData:
    def test_returns_dataframe_and_array(self) -> None:
        df, y = generate_sample_data(100)
        assert isinstance(df, pd.DataFrame)
        assert isinstance(y, np.ndarray)

    def test_row_count_matches_n(self) -> None:
...
class TestTrainModel:
    def test_returns_three_items(self, trained_models) -> None:
        assert len(trained_models) == 3

    def test_metrics_has_auc_mean(self, trained_models) -> None:
        _, _, metrics = trained_models
        assert "auc_mean" in metrics
        assert 0.0 <= float(metrics["auc_mean"]) <= 1.0

    def test_auc_mean_above_random(self, trained_models) -> None:
        _, _, metrics = trained_models
        assert float(metrics["auc_mean"]) > 0.55

    def test_n_features_in_metrics(self, trained_models) -> None:
        _, _, metrics = trained_models
        assert metrics["n_features"] == 18

    def test_both_models_fit(self, trained_models) -> None:
        xgb_pipe, lgbm_pipe, _ = trained_models
        assert xgb_pipe is not None
        assert lgbm_pipe is not None

    def test_xgb_can_predict_proba(self, trained_models, sample_booking_df) -> None:
...
class TestPredictDemand:
    def test_demand_score_in_range(self, trained_models, sample_booking_df) -> None:
        xgb_pipe, lgbm_pipe, _ = trained_models
        result = predict_demand(xgb_pipe, lgbm_pipe, sample_booking_df)
        assert 0.0 <= result["demand_score"] <= 1.0

    def test_suggested_rate_is_positive(self, trained_models, sample_booking_df) -> None:
        xgb_pipe, lgbm_pipe, _ = trained_models
        result = predict_demand(xgb_pipe, lgbm_pipe, sample_booking_df, base_rate=150.0)
        assert result["suggested_rate"] > 0

    def test_demand_tier_is_valid(self, trained_models, sample_booking_df) -> None:
        xgb_pipe, lgbm_pipe, _ = trained_models
        result = predict_demand(xgb_pipe, lgbm_pipe, sample_booking_df)
        assert result["demand_tier"] in {"low", "medium", "high"}

    def test_per_row_scores_length(self, trained_models, sample_booking_df) -> None:
        xgb_pipe, lgbm_pipe, _ = trained_models
        result = predict_demand(xgb_pipe, lgbm_pipe, sample_booking_df)
        assert len(result["per_row_scores"]) == len(sample_booking_df)

    def test_works_without_lgbm(self, trained_models, sample_booking_df) -> None:
        xgb_pipe, _, _ = trained_models
    ...
    def test_high_demand_gives_high_tier(self, trained_models) -> None:
        xgb_pipe, lgbm_pipe, _ = trained_models
    ...
    def test_low_demand_gives_low_tier(self, trained_models) -> None:
        xgb_pipe, lgbm_pipe, _ = trained_models
        from unittest.mock import patch
