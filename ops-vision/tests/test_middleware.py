...
class TestCorrelationIdMiddleware:
    ...
    def test_response_has_correlation_id_header(self, client) -> None:
        """Every response carries an X-Correlation-ID header."""
        resp = client.get("/ping")
        assert "X-Correlation-ID" in resp.headers

    def test_generated_id_is_non_empty(self, client) -> None:
        """A generated correlation ID is a non-empty string."""
        resp = client.get("/ping")
        assert len(resp.headers["X-Correlation-ID"]) > 0

    def test_incoming_correlation_id_is_preserved(self, client) -> None:
    ...
    def test_generated_ids_are_unique_per_request(self, client) -> None:
...
class TestCorrelationIdResponseTime:
    ...
    def test_response_has_response_time_header(self, client) -> None:
        """Every response carries an X-Response-Time-Ms header."""
        resp = client.get("/ping")
        assert "X-Response-Time-Ms" in resp.headers

    def test_response_time_is_positive_float(self, client) -> None:
        """X-Response-Time-Ms value is a parseable positive float."""
        resp = client.get("/ping")
        value = float(resp.headers["X-Response-Time-Ms"])
        assert value >= 0.0

    def test_response_time_differs_across_requests(self, client) -> None:
...
class TestRateLimitMiddleware:
    ...
    def test_forwarded_for_takes_first_address(self) -> None:
    ...
    def test_rate_limit_remaining_header_present(self) -> None:
        """Successful responses carry X-RateLimit-Remaining header."""
        client = TestClient(_build_app(RateLimitMiddleware, max_requests=5, window_seconds=60))
        resp = client.get("/ping")
        assert "X-RateLimit-Remaining" in resp.headers

    def test_rate_limit_remaining_decrements(self) -> None:
    ...
    def test_rate_limit_limit_header_matches_config(self) -> None:
    ...
    def test_429_includes_rate_limit_remaining_zero(self) -> None:
...
class TestRequestSizeLimitMiddleware:
    ...
    def test_413_response_has_detail_key(self) -> None:
